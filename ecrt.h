...
struct ec_slave_sii;
typedef struct ec_slave_sii ec_slave_sii_t; /**< \see ec_master */
...
int ecrt_master_get_slave_state(
        ec_master_t *master,
        uint16_t slave_position,
        uint8_t *al_state);

int ecrt_master_get_slave_count(ec_master_t *master);

#ifdef EC_HOT_PLUGGING
int ecrt_master_req_slave_state(ec_master_t *master, uint16_t slave_position, uint8_t al_state);
#endif
...
int ecrt_master_get_sync_manager(
		...
		uint8_t sync_index, /**< Sync manager index. Must be less
		                        than #EC_MAX_SYNC_MANAGERS. */
		ec_ioctl_sync_t *sync /**< Pointer to output structure. */
        );

int ecrt_master_set_sync_manager(
        ec_master_t *master, /**< EtherCAT master. */
        uint16_t slave_position, /**< Slave position. */
        uint8_t sync_index, /**< Sync manager index. Must be less
                                than #EC_MAX_SYNC_MANAGERS. */
...
#ifdef EC_HOT_PLUGGING
void ecrt_master_hot_activate(ec_master_t *master);

void ecrt_master_clear_config(ec_master_t *master, unsigned int slave_num);

void ecrt_master_app_pid(ec_master_t *master, pid_t pid);

void ecrt_master_hot_connect(ec_master_t *master, int is_hot_connect);
#endif
...
void ecrt_read_sii(ec_master_t *master,
        ec_slave_sii_t *data
        );

void ecrt_write_sii(ec_master_t *master,
        ec_slave_sii_t *data
        );
/******************************************************************************
