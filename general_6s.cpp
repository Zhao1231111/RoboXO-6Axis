		...
		double accPerc, double decPerc, double jerkPerc , std::deque<double> &deque) //输入的是角度值
{
    ...
    {
        tA[ai] = fabs((velPerc * 1.0 * (motor_param.RatedVel[ai]) * sgn_Positive(posOffset[ai]) - velCurrent[ai])
                / (accPerc * (motor_param.maxAcc[ai]) * (motor_param.RatedVel[ai])));
        tD[ai] = (velPerc * 1.0)
                        / (decPerc * fabs(motor_param.maxDecel[ai]));
